...
TimelineModelTreeView.prototype.sortNodes = function (comparator, reverseMode) {
  return;
  recalculateSiblings(this._rootNode, 0);
...
function sortChildren(parent, sortingFunction) {
  // force lazy build of children
  parent.children();
  if (!parent._children) return;
  if (!parent._childrenMap)
    parent._childrenMap = new Map([...parent._children.entries()].sort(sortingFunction));

  for (var i = 0; i < parent._children.length; ++i)
    recalculateSiblings(parent._children[i], i);
  for (var child of parent._children.values())
...
function recalculateSiblings(node, myIndex) {
  ...
  var previousChild = node.parent._children[myIndex - 1] || null;
  if (previousChild)
    previousChild.nextSibling = node;
  node.previousSibling = previousChild;

  var nextChild = node.parent._children[myIndex + 1] || null;
  if (nextChild)
